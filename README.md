...
Version 1.0 includes exact BigInt-backed decimal arithmetic, Regular/Medium/Light Freedom Pillar Wax color-strength presets at 0.50%/0.45%/0.40%, six manufacturer formula templates, a visual color wheel that shows the nearest W3C CSS named screen color, maps separately to the nearest predefined wax family, and applies bounded experimental ratio adjustments, custom ratios, linked range constraints, direct-dye calculation, optional Vybar dosing, fragrance-volume calculation from a US fl oz/lb bottle ratio, scale feasibility, printable results, manufacturer temperature-and-mixing guidance, and deterministic diagnostics. Color-wheel adjustments are illustrative screen-space heuristics—not calibrated predictions of cured wax—and never change the selected total dye load. The displayed process values are an unverified manufacturer transcription and do not replace wax-specific or equipment-specific safety instructions.
